...
useSortable(list, services, {
	...
	animation: 200,
	onUpdate: (event) => {
		moveArrayElement(services, event.oldIndex, event.newIndex, event);
				...
				<template v-if="services.length > 0">
					<ul ref="list" class="space-y-4">
						<template v-for="service in services" :key="service.id">
							<li>
								<div
									class="flex items-center h-12 px-2.5 py-1.5 bg-default border border-default rounded text-sm"
								>
									<div
